...
rng as (
  SELECT
	  ...
	  (sum(ideal_battery_range_km) / NULLIF(sum(coalesce(usable_battery_level,battery_level)), 0) * 100 ) AS "battery_rng"
  FROM (
    select battery_level, usable_battery_level, date, rated_battery_range_km, ideal_battery_range_km
    from positions
    where car_id = ${carId} and $__timeFilter(date) and ideal_battery_range_km is not null and battery_level>0
    union all
