...
SELECT
  ...
  -- with Postgres 12:
  date_trunc('${period}', start_date, '${timezone}') as local_period,
...
SELECT
  EXTRACT(EPOCH FROM date_trunc('${period}', local_period, '${timezone}'))*1000 AS date_from,
  EXTRACT(EPOCH FROM date_trunc('${period}', local_period AT TIME ZONE '${timezone}' + ('1 ' || '${period}')::INTERVAL, '${timezone}'))*1000 AS date_to,
  CASE '${period}'
    WHEN 'month' THEN to_char(local_period AT TIME ZONE '${timezone}', 'YYYY Month')
    WHEN 'year' THEN to_char(local_period AT TIME ZONE '${timezone}', 'YYYY')
    WHEN 'week' THEN 'week ' || to_char(local_period AT TIME ZONE '${timezone}', 'WW') || ' starting ' || to_char(local_period AT TIME ZONE '${timezone}', 'YYYY-MM-DD')
    ELSE to_char(local_period AT TIME ZONE '${timezone}', 'YYYY-MM-DD')
  END AS display,
