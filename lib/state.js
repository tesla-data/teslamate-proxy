...
const fields2 = 'odometer, inside_temp, driver_temp_setting, fan_status, is_climate_on, date';
...
exports.buildQuery = (carId) => [
  {
    ...
    refId: 'vs2',
    rawSql: `SELECT ${fields2} FROM (
      (SELECT ${fields2} FROM positions WHERE date > CURRENT_DATE - INTERVAL '30 days' AND car_id = ${carId} ORDER BY date DESC LIMIT 1) UNION
      (SELECT ${fields2} FROM drives LEFT JOIN positions ON positions.id = end_position_id WHERE car_id = ${carId} ORDER BY drives.id DESC LIMIT 1)
    ) AS data ORDER BY date DESC LIMIT 1;`
  }, {
    refId: 'charge',
    rawSql: `
