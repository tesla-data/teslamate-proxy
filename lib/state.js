...
exports.buildQuery = (carId) => [
  {
    ...
    rawSql: `SELECT odometer, inside_temp, driver_temp_setting, fan_status, is_climate_on, date FROM (
      (SELECT * FROM positions WHERE date > CURRENT_DATE - INTERVAL '30 days' AND car_id = ${carId} ORDER BY date DESC LIMIT 1)
      UNION (SELECT positions.* FROM drives LEFT JOIN positions ON positions.id = drives.end_position_id WHERE drives.car_id = ${carId} ORDER BY drives.id DESC LIMIT 1)
    ) AS data ORDER BY date DESC LIMIT 1;`
  }, {
    refId: 'charge',
    rawSql: `
